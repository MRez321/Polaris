...
- **Workshop todo (کار کارگاه)** — checklist item on the workshop dashboard. Priority: low (کم) / medium (متوسط) / high (زیاد) / urgent (فوری), optional due date (past due = «سررسید گذشته» chip + counter in widget header), done toggle with doneAt. Color accents: sky/amber/rose/red. Clear-done bulk action. Pure workshop-internal — no link to orders/consignments.
- **Backup (پشتیبان)** — archive of database and/or website files produced by the backup engine. Kinds: database (`mysqldump` → `.sql.gz`), website (backend source tar.gz), full (both). Automatic runs (schedule + retention) prefix `auto-`; optional cPanel host-side full backup via UAPI; optional Telegram announcement on completion. Managed in Settings → پشتیبان‌گیری.
- **Telegram relay (رله تلگرام)** — Cloudflare Worker mirroring api.telegram.org so Telegram notifications work from Iranian servers without a proxy. `relayUrl` setting takes precedence over `proxyUrl`.
...
Dashboard: total active debt, overdue debt, today's payments, inventory value (stock × cost), items in sellers' hands, low-stock alerts, workshop costs, collected total, net profit (collected − expenses). Dashboard layout v2 (16 reorderable/collapsible widgets with col-span and presets, see FRONTEND.md) adds: live Jalali clock, colorful todo list, overdue alerts banner, latest shop sales, latest seller income, latest returns, most-sold items, scheduled deliveries (تحویل‌های زمان‌بندی‌شده), pending-production items, workshop liquid balance, 7/30-day income window.
