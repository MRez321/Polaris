...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, overdueCount }) => {
  // Relocated and consolidated navigation
  const navItems = [
    { id: 'dashboard', label: 'پیشخوان و داشبورد', icon: LayoutDashboard },
    { id: 'inventory', label: 'انبار و موجودی پوشاک', icon: Package },
    {
      id: 'consignments',
    ...
    { id: 'people', label: 'فروشندگان و پرسنل کارگاه', icon: Users },
    { id: 'finances', label: 'امور مالی، درآمد و هزینه‌ها', icon: CreditCard },
    { id: 'settings', label: 'تنظیمات، ممیزی و سامانه', icon: Settings },
  ];
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs transition-all text-right ${
                isActive
            ...
            >
              <div className="flex items-center gap-3">
                <Icon className={`w-4 h-4 shrink-0 ${isActive ? 'text-black' : 'text-[#A67C38] dark:text-[#CEAE80]'}`} />
                <span className={isActive ? 'text-black' : 'text-stone-800 dark:text-gray-200'}>{item.label}</span>
              </div>
