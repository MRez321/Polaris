...
export const WorkshopManager: React.FC<WorkshopManagerProps> = ({
  ...
  const fetchWorkshopData = async () => {
    try {
      ...
      const [expRes, profitRes] = await Promise.all([
        fetch('/api/workshop/expenses').then((r) => (r.ok ? r.json() : [])),
        fetch('/api/workshop/profit-distribution').then((r) => (r.ok ? r.json() : [])),
      ]);
      setExpenses(expRes);
  ...
  const handleSaveExpense = async (e: React.FormEvent) => {
    ...
    try {
      if (editingExpense) {
        const res = await fetch(`/api/workshop/expenses/${editingExpense.id}`, {
          method: 'PUT',
        ...
        const res = await fetch('/api/workshop/expenses', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleSaveDistribution = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/workshop/profit-distribution', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
