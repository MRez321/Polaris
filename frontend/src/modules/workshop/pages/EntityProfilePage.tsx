...
export const EntityProfilePage: React.FC = () => {
  ...
  const itemView = useMemo(() => {
    ...
    entries.push({
      ...
      description: `قیمت تمام‌شده دوخت: ${formatToman(item.costPrice)}`,
    });
      ...
      return (
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
            ...
            <StatBox label="قیمت امانی / خرده‌فروشی">
              <span className="font-mono text-xs">
                {formatToman(item.consignmentPrice)} <span className="text-stone-400">/</span>{' '}
            ...
            {(s.bankAccounts || []).length > 0 && (
              <ContactRow icon={<CreditCard className="w-3.5 h-3.5" />}>
                <div className="space-y-1">
                  {s.bankAccounts && s.bankAccounts.map((acc, i) => (
                    <div key={acc.id || i} className="font-mono text-[11px]" dir="ltr">
                      {[acc.cardNumber, acc.shebaNumber].filter(Boolean).join(' • ') || acc.bankName}
                      {acc.bankName ? <span className="font-sans text-stone-400"> — {acc.bankName}</span> : null}
                    </div>
                  ))}
                </div>
              </ContactRow>
      ...
      <>
        ...
        {(o.bankAccounts || []).length > 0 && (
          <div className="pt-1 border-t border-black/5 dark:border-white/5">
            <ContactRow icon={<CreditCard className="w-3.5 h-3.5" />}>
              <div className="space-y-1">
                {o.bankAccounts.map((acc, i) => (
                  <div key={acc.id || i} className="font-mono text-[11px]" dir="ltr">
                    {[acc.cardNumber, acc.shebaNumber].filter(Boolean).join(' • ')}
                    {acc.bankName ? <span className="font-sans text-stone-400"> — {acc.bankName}</span> : null}
                  </div>
                ))}
              </div>
