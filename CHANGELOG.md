...
- **Security hardening (P0-A)**: TOTP two-factor authentication (better-auth `twoFactor()`; migration 0016 `two_factor` + `user.two_factor_enabled`; challenge flow on login; encrypted secrets + backup codes; settings panel in workshop Settings → Security); API rate limiting with Persian 429 + `Retry-After` (auth 10/15min keyed IP+email, MFA 10/min, sensitive 20/min, general 180/min; `/api/health` exempt); HTTP security headers + CSP (`core/middleware/securityHeaders.ts` — nosniff, frame-ancestors 'none', HSTS prod-HTTPS only, x-powered-by removed; browser-verified); server-side credential masking (Telegram botToken / SMS key / cPanel token returned as 16 bullets, never serialized; exact-mask resubmits skipped so secrets survive saves) + sanitized log sites; RBAC permission catalog on backup/notification routes; backup concurrency lock (409 on parallel runs), download audit logging, traversal-safe ids, and mysqldump DB password via `MYSQL_PWD` env (never argv). Regression suite: `backend/scripts/smoke-security.mjs` (44 checks). Security docs: `docs/security/`. Dependency audit: multer 2.2.0 → 2.3.0 (high-severity DoS fixes) + transitive patches.
